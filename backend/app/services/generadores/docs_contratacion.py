...
def generar_autorizacion_inicio(ctx):
    ruta = os.path.join(RUTA_RESULTADOS, f"{ctx['proceso'].nro_orden or ctx['proceso'].id}_3_AutorizacionContratacion.xlsx")
...
def generar_informe_cotizacion(ctx):
    ruta = os.path.join(RUTA_RESULTADOS, f"{ctx['proceso'].nro_orden or ctx['proceso'].id}_5_InformeCotizacion.docx")
...
def generar_orden_compra(ctx):
    ruta = os.path.join(RUTA_RESULTADOS, f"{ctx['proceso'].nro_orden or ctx['proceso'].id}_4_OrdenCompra.xlsx")
...
def generar_notificacion_adjudicacion(ctx):
    ruta = os.path.join(RUTA_RESULTADOS, f"{ctx['proceso'].nro_orden or ctx['proceso'].id}_5_NotificacionAdjudicacion.docx")
