...
from migrar_unidades import migrar_cargos_a_unidades
...
def poblar_usuarios():
    ...
    try:
        # Inyectar las nuevas columnas en PostgreSQL por si no existen
        try:
            ...
            db.execute(text("ALTER TABLE unidades ADD COLUMN IF NOT EXISTS responsable_id INTEGER;"))
            db.commit()
        except Exception as e:
            db.rollback()

        # Limpiar referencias previas de responsable_id para evitar errores FK al vaciar usuarios
        ...
        print("Inyectando usuarios iniciales...")
        ...
        usuarios_seed = [
            ...
            Usuario(username="pasante.obras", password_hash=password_default, titulo="", nombre_completo="Juan Pérez (Pasante)", cargo="Responsable de Obras Públicas y Desarrollo Territorial", rol="PASANTE"),
        ...
        print("Ejecutando migración automática de cargos a unidades...")
